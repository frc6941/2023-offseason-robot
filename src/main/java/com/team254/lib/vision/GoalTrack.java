...
public class GoalTrack {
    ...
    public synchronized boolean tryUpdate(double timestamp, Pose2d new_observation) {
        ...
        if (distance < Constants.VisionConstants.TRACK_MAX_DISTANCE) {
    ...
    synchronized void pruneByTime() {
        double delete_before = Timer.getFPGATimestamp() - Constants.VisionConstants.TRACK_MAX_AGE;
    ...
    synchronized void smooth() {
        if (isAlive()) {
            ...
            for (Map.Entry<Double, Pose2d> entry : mObservedPositions.entrySet()) {
                if (t_now - entry.getKey() > Constants.VisionConstants.TRACK_MAX_SMOOTHING_TIME) {
                    continue;
                }
                ++num_samples;
    ...
    public synchronized double getStability() {
        return Math.min(1.0, mObservedPositions.size() / (Constants.VisionConstants.FRAME_RATE * Constants.VisionConstants.TRACK_MAX_AGE));
    }

...
public class SwerveLocalizer implements Localizer {
    ...
    public SwerveLocalizer(SwerveDriveKinematics kinematics, int poseBufferSize, int velocityBufferSize, int accelerationBufferSize) {
    ...
    @Override
    public synchronized Pose2d getPredictedVelocity(double lookahead) {
        synchronized(statusLock) {
            return vehicleVelocityMeasured.plus(
                new Transform2d(
                    vehicleAccelerationMeasured.getTranslation().times(lookahead),
                    vehicleAccelerationMeasured.getRotation().times(lookahead)
                )
            );
        }
    }

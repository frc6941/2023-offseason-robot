...
public class Vision implements Updatable {
    ...
    private void updateGoalTracker(double time, List<TargetInfo> observations) {
        ...
        edu.wpi.first.math.geometry.Pose2d currentPose = Swerve.getInstance().getLocalizer().getPoseAtTime(time);
    ...
    @Synchronized
    public synchronized Optional<TrackReport> getTrack(double time, int previousId) {
    ...
    public void read(double time, double dt) {
        ...
        mPeriodicIO.givenLedMode = (int) mNetworkTable.getEntry("ledMode").getDouble(3.0);
